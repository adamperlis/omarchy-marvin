...
Item {
  ...
  function handleActiveSignal() {
    ...
    // Upstream watches for a Hyprland window with class org.omarchy.screensaver,
    // because its screensaver is a terminal. Marvin's is a layer-shell surface,
    // which emits no openwindow/closewindow at all, so that count never leaves
    // zero and the first stray activity would tear the cycle down before the
    // lock deadline. The launcher runs qs in the foreground, so this process is
    // alive for exactly as long as the screensaver is on screen — which is the
    // same signal, read directly rather than inferred from the window list.
    if (root.screensaverStartedThisCycle && (screensaverProcess.running || root.screensaverWindowCount > 0 || screensaverLaunchGraceTimer.running)) {
