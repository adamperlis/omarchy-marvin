...
//   - rows separate by whitespace, never a divider
//   - tone is a surface property: [marvin-weather] in shell.toml tints the
//     card; without it the panel takes the ordinary popup surface
Panel {
  id: root
  ...
  // Tone. A theme may ship a [marvin-weather] section in shell.toml with
  // background / text / muted; every value is a literal hex. Without it the
  ...
  KeyboardPanel {
    ...
    // padding, same radius, drawn beneath the content.
    Rectangle {
      anchors.fill: parent
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          // ---- Location. Tap to edit; the field replaces the label in place.
          Item {
            width: parent.width
            ...
            Row {
              ...
              TextField {
                id: locationField
                width: parent.width - clearLocation.width - Style.spacing.sm
          ...
          // ---- Geocoding suggestions while editing. 32px rows, fill for the cursor row.
          Column {
          ...
          // ---- Hero. The number is the biggest thing on the card; the unit hangs
          //      off its top as a caption; the condition glyph sits at the far edge.
          Item {
            visible: !!root.current
            width: parent.width
            height: tempBig.implicitHeight
          ...
          // ---- Now. Value over label, three columns, whitespace between.
          Row {
            visible: !!root.current
            spacing: Style.spacing.xxl
          ...
          // ---- Next days. Day, glyph, high, low. No rule above it; the gap is the rule.
          Row {
