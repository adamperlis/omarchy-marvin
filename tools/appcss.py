...
def oncolor(c, h):
    """The palette's near-black or near-white, whichever reads better on h."""
    dark, light = c["darker_background"], c["bright_foreground"]
    return dark if _contrast(dark, h) >= _contrast(light, h) else light

def gtk(c):
    acc, accfg = c["accent"], oncolor(c, c["accent"])
    red, green, yellow = c["red"], c["green"], c["yellow"]
    return f"""/* Marvin for GTK 3 and 4 / libadwaita. Generated by tools/appcss.py from
   colors.toml. Named-colour overrides only — no widget layout is touched — so
   file choosers, save dialogs and GTK apps take Marvin's palette without
   fighting each app's own structure. Installed to ~/.config/gtk-3.0 and
   gtk-4.0 by the config layer, and re-copied per tone by marvin-mode. */
...
@define-color headerbar_bg_color {c['lighter_background']};
...
@define-color dialog_bg_color {c['lighter_background']};
...
@define-color sidebar_bg_color {c['background']};
@define-color sidebar_fg_color {c['foreground']};
@define-color destructive_color {red};
@define-color destructive_bg_color {red};
@define-color destructive_fg_color {oncolor(c, red)};
...
@define-color theme_base_color {c['background']};
@define-color theme_text_color {c['foreground']};
@define-color theme_selected_bg_color {acc};
@define-color theme_selected_fg_color {accfg};
@define-color theme_unfocused_bg_color {c['background']};
@define-color theme_unfocused_fg_color {c['foreground']};
@define-color theme_unfocused_base_color {c['background']};
@define-color theme_unfocused_text_color {c['foreground']};
@define-color insensitive_bg_color {c['background']};
...
/* Force the serif on the note itself — reading view and both editors — so it
   applies even when a "Text font" is set in Appearance (which otherwise wins
   over --font-text-theme via --font-text). Point --font-text at the serif too,
   for anything that reads the variable directly. */
body {{ --font-text: var(--font-text-theme); }}
.markdown-rendered, .markdown-preview-view,
.markdown-source-view.mod-cm6 .cm-content, .cm-line, .cm-editor {{
  font-family: var(--font-text-theme) !important;
}}

/* Headings share the serif of the body — hierarchy is size and weight, not a
   change of voice or colour. h1 and h2 are tracked in slightly. */
...
.HyperMD-header, .cm-header {{ font-family: var(--font-text-theme); }}
...
if __name__ == "__main__":
    for light, pre in ((False, ""), (True, "light/")):
        ...
        (ROOT / f"{pre}gtk.css").write_text(gtk(c))
        print("wrote", f"{pre}{{obsidian.css, hyprland-preview-share-picker.css, btop.theme, chromium.theme, helix.toml, gtk.css}}")
