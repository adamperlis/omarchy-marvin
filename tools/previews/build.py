...
.card{background:var(--raised);border-radius:24px;padding:24px;display:flex;flex-direction:column;gap:24px;border:1px solid rgba(var(--fg-rgb),.08);box-shadow:0 8px 32px rgba(0,0,0,var(--sh))}
...
.card .now{display:flex;gap:12px;align-items:center}.card .now .art{width:96px;height:96px;border-radius:12px;overflow:hidden;background:rgba(var(--fg-rgb),.06);display:flex;align-items:center;justify-content:center;color:var(--muted)}
.card .now .tt{display:flex;flex-direction:column;gap:4px}.card .now .tt b{font-size:18px;font-weight:500}.card .now .tt i{font-style:normal;color:var(--muted)}.card .now .tt small{font-size:12px;color:var(--muted)}
...
.mood{display:grid;grid-template-columns:1fr 1fr;gap:8px}.mood span{height:112px;border-radius:12px;display:block}.wx .hero .cap span,.wx .stats span,.wx .days span{color:var(--wx-muted)}.wx .stats b{color:var(--wx-ink)}
...
.launcher{background:var(--raised);border-radius:24px;padding:24px;display:flex;flex-direction:column;gap:16px;border:1px solid rgba(var(--fg-rgb),.08);box-shadow:0 8px 32px rgba(0,0,0,var(--sh))}
.field{height:40px;border-radius:20px;background:rgba(var(--fg-rgb),.06);display:flex;align-items:center;gap:8px;padding:0 14px;border:2px solid var(--accent)}.field i{font-style:normal;color:var(--muted)}
...
.note{background:var(--raised);border-radius:24px;padding:24px;display:flex;flex-direction:column;gap:8px;line-height:1.5;border:1px solid rgba(var(--fg-rgb),.08);box-shadow:0 8px 32px rgba(0,0,0,var(--sh))}
...
.osd{width:280px;height:56px;background:var(--raised);border-radius:24px;display:flex;align-items:center;gap:16px;padding:0 24px;border:1px solid rgba(var(--fg-rgb),.08);box-shadow:0 8px 32px rgba(0,0,0,var(--sh))}.osd .hair{flex:1}.osd .n{font-size:16px;font-weight:500;width:2ch;text-align:right}
"""

def vars_css(t):
    return f":root{{--bg:{t['bg']};--raised:{t['raised']};--fg:{t['fg']};--fg-rgb:{hexrgb(t['fg'])};--muted:{t['muted']};--accent:{t['accent']};--attn:{t['attn']};--wx-bg:{t['wx'][0]};--wx-ink:{t['wx'][1]};--acc-rgb:{hexrgb(t['accent'])};--wx-muted:{t['wx'][2]};--wx-end:{t['wx'][3]};--pw-bg:{t['pw'][0]};--pw-ink:{t['pw'][1]};--pw-rgb:{hexrgb(t['pw'][1])};--pw-muted:{t['pw'][2]};--chip-fill:{t['pw'][3]};--chip-text:{t['pw'][4]};--sh:{'.06' if t['light'] else '.28'}}}"
...
<div class="pills"><span class="l">Power profile</span><div><em>{ic("eco")}Saver</em><em class="on">{ic("balance")}Balanced</em><em>{ic("bolt")}Performance</em></div></div></div>''',
     "media": f'''<div class="card" style="width:360px;gap:16px"><div class="now"><div class="art" style="background:url('file://{t["walls"][1][1]}') 30% 40%/300% auto"></div><div class="tt"><b>The Visit</b><i>Agar Agar</i><small>The Dog and the Future</small></div></div>
...
.win{{position:absolute;background:var(--bg);border-radius:24px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,var(--sh)),0 0 0 1px rgba(var(--fg-rgb),.08)}}
...
def sheet(t, fonts):
    ...
    cols = [
        cell("Music", w["media"]) + cell("Launcher", w["launcher"]) + cell("Volume", w["osd"]),
        cell("Battery", w["power"]) + cell("Weather", w["weather"]),
        cell("Calendar", w["clock"]) + cell("Notification", w["note"]) + cell("Backgrounds", w["mood"]),
    ]
    return f"""<!doctype html><html><head><meta charset="utf-8"><style>{font_face(fonts)}{vars_css(t)}{BASE_CSS}
html,body{{width:1600px;height:1360px;overflow:hidden}}body{{background:{ground};padding:56px 0}}
.g{{display:grid;grid-template-columns:360px 360px 360px;gap:0 56px;justify-content:center;align-items:start}}
...
.card,.note,.launcher{{width:360px}}
...
if __name__ == "__main__":
    ...
    for light in (False, True):
        ...
        for name, html, size, dests in (
            ...
            ("widgets", sheet(t, a.fonts), (1600, 1360), [out / f"docs/images/widgets-{tag}.png"]),
